 ...
 * @Last Modified time: 2019-09-06 16:13:54
 * @Func 试题相关的控制器
 */
...
class QuestionsController extends Controller {
    ...
    async update() {
        if (this.ctx.params.id === 'update') {
            ...
            if (
                ...
                let result = await this.ctx.service.exam.questions.updateQuestion(
                    questions_id,
                    update
                )
                if (result === 1) {
    ...
    async show() {
        if (this.ctx.params.id === 'condition') {
            ...
            let result = await this.ctx.service.exam.questions.getConditionQuestions(
                conditions
            )
